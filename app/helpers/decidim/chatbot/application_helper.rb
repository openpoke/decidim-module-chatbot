...
module Decidim
  module Chatbot
    module ApplicationHelper
      ...
      def components_for_space(space_gid, organization)
        return [] if space_gid.blank?

        space = organization.public_participatory_spaces.find { |s| s.to_gid.to_s == space_gid }
        return [] unless space

        space.components.published.where(manifest_name: "proposals").map do |c|
          { id: c.id.to_s, name: translated_attribute(c.name) }
        end
      end

      def components_for_space_json(organization)
        organization.public_participatory_spaces.each_with_object({}) do |space, hash|
          components = space.components.published.where(manifest_name: "proposals")
